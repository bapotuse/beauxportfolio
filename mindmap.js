...
stage.on("contextmenu",(e) => {
	...
	makeContextMenu(e,`
		<ul>
			<li onclick="ctxAddNode()">Ajouter un noeud</li>` + customContext + `
		<ul>
	`);
	}
});
...
function selectNode(e){
	if(currentRect){
		currentRect.stroke("black");
		unhighlightSkillLines(currentRect.getAttr('linkedSkill'));
	}
	currentRect = e.target;
	currentRect.stroke("red");
	currentSkill = e.target.getAttr('linkedSkill');
	highlightSkillLines(currentSkill);
	document.forms["skill"].name.value = currentSkill.name;
	document.forms["skill"].style.display = "flex";
...
function makeSkill(posX = - stage.x() / stage.scaleX() + stage.width() / (2 * stage.scaleX())  ,posY = -stage.y() / stage.scaleY()  + stage.height() / (2 * stage.scaleY()),skill = null){
	...
	circle.on('contextmenu',(e) => {
		...
		makeContextMenu(e, `
		<ul>
			<li onclick="ctxAddParent()">Ajouter un parent</li>
			<li onclick="ctxAddChild()">Ajouter un enfant</li>
			<li onclick="deleteSkill()">Supprimer</li>`
		+
			customContext
		+
		`
